...
function normalizeAnnouncement(row: UnknownRow): SessionAnnouncementRecord | null {
  ...
  const body = sanitizeText(readString(row, "body"), 280);
  ...
  return {
    id,
    sessionId,
    title: sanitizeText(readString(row, "title"), 80) || undefined,
    body,
...
export async function createSessionAnnouncement(args: {
  ...
  const announcement = {
    ...
    title: sanitizeText(args.title, 80) || null,
    body: sanitizeText(args.body, 280),
    type: args.type ?? "text",
    audio_url: sanitizeText(args.audioUrl, 500) || null,
...
export async function fetchSessionEngagement(
  ...
  const [heartbeatsResult, reactionsResult, pollResponsesResult, questionsResult, participantsResult] =
    await Promise.all([
      supabase
  ...
  const heartbeatRows = (heartbeatsResult.data as UnknownRow[] | null) ?? [];
  const participantRows = (participantsResult.data as UnknownRow[] | null) ?? [];
  ...
  (((reactionsResult.data as UnknownRow[] | null) ?? [])).forEach((row) => {
    const anonymousId = readString(row, "anonymous_id");
    if (!anonymousId) {
      return;
    }
  ...
  (((pollResponsesResult.data as UnknownRow[] | null) ?? [])).forEach((row) => {
    const anonymousId = readString(row, "anonymous_id");
    if (!anonymousId) {
      return;
    }
  ...
  (((questionsResult.data as UnknownRow[] | null) ?? [])).forEach((row) => {
    const anonymousId = readString(row, "anonymous_id");
    if (!anonymousId) {
      return;
    }
